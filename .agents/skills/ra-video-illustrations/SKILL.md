...
八个中文调用入口和四个 V4 主题见 [theme-library.md](references/theme-library.md)。类型、风格与模式见 [style-profiles.md](references/style-profiles.md)；V3 判断顺序必须使用 [semantic-routing-v3.md](references/semantic-routing-v3.md)。
...
5. 所有精确中文标签由 renderer 根据 `overlay_labels` 绘制，不能让图片模型生成标题、字幕、长段文字或程序 UI。
